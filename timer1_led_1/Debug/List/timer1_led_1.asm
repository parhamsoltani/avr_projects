...
; 0000 0024       PORTC ^= (1<<0);
	...
	EOR  R30,R26
	OUT  0x15,R30
; 0000 0025       TCNT1 = remainder_time;
	RCALL SUBOPT_0x0
; 0000 0026 
; 0000 0027 }
	LD   R30,Y+
...
_timer1_init:
; .FSTART _timer1_init
; 0000 002F     long int timer_clock = mcu_clock/prescaler;
; 0000 0030     float counting_time = 1000.0/timer_clock;
; 0000 0031     float overflow_time = counting_time*65535;
; 0000 0032 
; 0000 0033     long int x = req_delay/counting_time;
; 0000 0034 
; 0000 0035     if (x > 65535)
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,16
;	mcu_clock -> Y+20
...
;	x -> Y+0
	...
	BRLT _0x3
; 0000 0036     {
; 0000 0037         tot_overflow += (x/65535);
	__GETD1N 0xFFFF
	CALL __DIVD21
	LDS  R26,_tot_overflow
	LDS  R27,_tot_overflow+1
	ADD  R30,R26
	ADC  R31,R27
	STS  _tot_overflow,R30
	STS  _tot_overflow+1,R31
; 0000 0038         remainder_time = 65536 - ((unsigned int)x%65535);
	LD   R26,Y
	LDD  R27,Y+1
	LDI  R30,LOW(65535)
	LDI  R31,HIGH(65535)
	...
	SUB  R26,R30
	SBC  R27,R31
	STS  _remainder_time,R26
	STS  _remainder_time+1,R27
; 0000 0039     }
; 0000 003A     else
	RJMP _0x4
_0x3:
; 0000 003B     {
; 0000 003C         remainder_time = 65535 - x;
	LD   R26,Y
	LDD  R27,Y+1
	LDI  R30,LOW(65535)
	LDI  R31,HIGH(65535)
	SUB  R30,R26
	SBC  R31,R27
	STS  _remainder_time,R30
	STS  _remainder_time+1,R31
; 0000 003D     }
_0x4:
; 0000 003E 
; 0000 003F     TCCR1B |= (1 << CS12);
	IN   R30,0x2E
	ORI  R30,4
	OUT  0x2E,R30
; 0000 0040     TCNT1 = remainder_time;
	RCALL SUBOPT_0x0
; 0000 0041     TIMSK |= (1 << TOIE1);
	IN   R30,0x39
	ORI  R30,4
	OUT  0x39,R30
; 0000 0042 }
...
; 0000 004B 
; 0000 004C int prescaler = 256;
; 0000 004D int req_delay = 1000;
; 0000 004E float mcu_clock = 8000000.0;
; 0000 004F DDRC |= (1 << 0);
	SBIW R28,4
	LDI  R30,LOW(0)
	ST   Y,R30
...
;	mcu_clock -> Y+0
	...
	SBI  0x14,0
; 0000 0050 
; 0000 0051 timer1_init(mcu_clock, prescaler, req_delay);
	CALL __GETD1S0
	CALL __PUTPARD1
	ST   -Y,R17
	ST   -Y,R16
...
; 0000 0061 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=Out
; 0000 0062 DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (0<<DDC1) | (1<<DDC0);
	LDI  R30,LOW(1)
	OUT  0x14,R30
; 0000 0063 // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=0
...
SUBOPT_0x0:
	LDS  R30,_remainder_time
	LDS  R31,_remainder_time+1
	OUT  0x2C+1,R31
	OUT  0x2C,R30
	RET
