...
;NAME DEFINITIONS FOR GLOBAL VARIABLES ALLOCATED TO REGISTERS
	.DEF _tot_overflow=R4
	.DEF _tot_overflow_msb=R5
...
; 0000 0025 ++tot_overflow;
	MOVW R30,R4
	ADIW R30,1
	MOVW R4,R30
; 0000 0026 //for 8MHz MCU clock and 1024 prescaler, the overflow time for 8-Bit Timer0 is 32.7659ms.
; 0000 0027 if (tot_overflow >= 30)  //required delay is one second, so 30 overflows will happen.
	LDI  R30,LOW(30)
	LDI  R31,HIGH(30)
	CP   R4,R30
	CPC  R5,R31
	BRLO _0x3
; 0000 0028 {
; 0000 0029   //remaining time would be 17.023ms which requires almost 133 counting process.
; 0000 002A   //TCNT0 would start from 255-133=122 which is 0x7A to 0xFF for the 31th overflow.
; 0000 002B   TCNT0=0x7A;
	LDI  R30,LOW(122)
	OUT  0x32,R30
; 0000 002C   if (tot_overflow >= 31)
	LDI  R30,LOW(31)
	LDI  R31,HIGH(31)
	CP   R4,R30
	CPC  R5,R31
...
; 0000 003B  TCNT0=0x00;
	LDI  R30,LOW(0)
	OUT  0x32,R30
; 0000 003C  TIMSK |= (1<<TOIE0);
	IN   R30,0x39
	ORI  R30,1
	OUT  0x39,R30
; 0000 003D 
; 0000 003E  tot_overflow = 0;
	CLR  R4
	CLR  R5
; 0000 003F }
	RET
...
; 0000 0056 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=Out
; 0000 0057 DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (0<<DDC1) | (1<<DDC0);
	LDI  R30,LOW(1)
	OUT  0x14,R30
; 0000 0058 // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=0
...
; 0000 006C TIMSK=(0<<OCIE2) | (0<<TOIE2) | (0<<TICIE1) | (0<<OCIE1A) | (0<<OCIE1B) | (0<<TOIE1) | (0<<OCIE0) | (1<<TOIE0);
	LDI  R30,LOW(1)
	OUT  0x39,R30
; 0000 006D 
; 0000 006E // Global enable interrupts
; 0000 006F 
; 0000 0070 #asm("sei") // SREG=SREG | 0x80; is also possible
	sei
; 0000 0071 
; 0000 0072 while (1)
_0x5:
